...
class SocializeManager extends Manager implements Contracts\Factory
{
     ...
     *
     */
    protected function createKakaotalkDriver()
    {
        $config = $this->app['config']['services.kakaotalk'];

        return $this->buildProvider(
            'Nanuly\Socialize\Two\KakaotalkProvider', $config
        );
    }
    ...
    {
        $config = $this->app['config']['services.naver'];

        return $this->buildProvider(
            'Nanuly\Socialize\Two\NaverProvider', $config
        );
    }
